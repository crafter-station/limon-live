...
| --- | --- | --- | --- | --- | --- |
| Candidate image identity and count | Menu processor accepts retained place photos only and caps candidates with a typed server constant | Retained Blob URLs become image parts | Extra photos, reviewer avatars, review imagery | Unit test exact URLs and cap | No OCR of reviews or provider hotlinks |
...
| Timeout, retry, output, concurrency bounds | Server constants and abort signal wrap one bounded processor | Valid response within limits publishes | Timeout, transient failure, oversized response, concurrent request | Fake executor tests attempts, abort, size, semaphore | No environment overrides |
...
| Visibly grounded anti-invention | Output carries source-image evidence tokens; validator requires each visible string in supplied OCR transcription/evidence | Exact source spelling survives | Invented item/price/description rejects | Grounding unit tests | Model confidence is not proof |
