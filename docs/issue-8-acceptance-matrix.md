...
| Visibly grounded anti-invention | Output carries source-image evidence tokens; validator requires item fields, each non-null price currency as a standalone marker, and section names in supplied transcription/evidence | Exact source spelling survives | Invented item/price/currency/description/section rejects | Grounding unit tests, including item and variant `PENNE` boundaries | Model transcription is not cryptographic pixel grounding |
...
| PEN normalization | Every non-null item and variant currency must be visibly grounded as S/, S/. or a standalone PEN marker before normalization; amount stays decimal string | Visible `PEN`, `S/`, and `S/.` prices render as PEN | Missing, invented, dollar, or ambiguous substring currency rejects publication | Item and variant normalization tests | No currency conversion |
