...
type MenuExecutor = (photos: readonly Photo[]) => Promise<MenuExtraction>;
...
export class MenuExtractor {
  constructor(private readonly execute: MenuExecutor) {}
  ...
  async extract(photos: readonly Photo[]): Promise<Menu | null> {
    ...
    const output = menuExtractionSchema.parse(
      await withConcurrency(() => this.execute(candidates)),
    );
...
export function createGatewayMenuExtractor(apiKey: string) {
  ...
  return new MenuExtractor(async (photos) => {
